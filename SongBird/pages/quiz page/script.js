const birdsData = [
  ...
  [
    ...
    },
    {
      ...
      image: 'https://erbirds.ru/taxons/16/fronpic2.jpg',
...
const listItemVariants = document.querySelectorAll('.list-item-variant');
const randomImg = document.querySelectorAll('.random-img');
const listItemCardName = document.querySelector('.list-item-card-name');
const listItemCardSecondName = document.querySelector('.list-item-card-secondName');
const cardInfoDescr = document.querySelector('.card-info-descr');
...
function createAnswerVariants(){
  ...
  answers[i].textContent = birdsData[answerTime][i].name
}
}


function randomNumber(a,b){return Math.floor(Math.random() * (b - a + 1)) + a
}
...
function openDetailsInfo(choice) {
  randomImg[1].src = birdsData[answerTime][choice].image;
  listItemCardSecondName.textContent = birdsData[answerTime][choice].species;
  listItemCardName.textContent = birdsData[answerTime][choice].name;
  cardInfoDescr.textContent = birdsData[answerTime][choice].description;
}
...
function clearStyles(){
  ...
  for (i=0;i<circles.length;i++){
    circles[i].style.background = 'azure';
  }
  for (i=0;i<answers.length;i++){
    answers[i].style.color = 'azure';
  }
  randomImg[0].src = '../../assets/images/unknownbird.jpg';
}